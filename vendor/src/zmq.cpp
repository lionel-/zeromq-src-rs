...
int zmq_poll (zmq_pollitem_t *items_, int nitems_, long timeout_)
{
    ...
    int nevents = 0;
    // Trace single-socket polls with non-zero timeout (the case that hangs
    // on Windows ARM64)
    const bool _trace = (nitems_ == 1 && timeout_ != 0 && items_[0].socket);
    int _iter = 0;
    if (_trace)
        fprintf (stderr, "ZMQ_POLL_TRACE: enter nitems=%d timeout=%ld\n",
                 nitems_, timeout_);

    while (true) {
        if (_trace)
            fprintf (stderr, "ZMQ_POLL_TRACE: loop iter=%d first_pass=%d\n",
                     _iter++, (int) first_pass);
#if defined ZMQ_POLL_BASED_ON_POLL
        ...
        {
            if (_trace)
                fprintf (stderr, "ZMQ_POLL_TRACE: WSAPoll fd=%lld timeout=%d\n",
                         (long long) pollfds[0].fd, (int) timeout);
            const int rc = poll (&pollfds[0], nitems_, timeout);
            if (_trace)
                fprintf (stderr, "ZMQ_POLL_TRACE: WSAPoll rc=%d revents=0x%x\n",
                         rc, (unsigned) pollfds[0].revents);
            if (rc == -1 && errno == EINTR) {
                return -1;
            }
            errno_assert (rc >= 0);
        ...
        for (int i = 0; i != nitems_; i++) {
            ...
            if (items_[i].socket) {
                ...
                if (_trace && i == 0)
                    fprintf (stderr, "ZMQ_POLL_TRACE: ZMQ_EVENTS=0x%x revents=0x%x\n",
                             zmq_events, (unsigned) items_[i].revents);
            }
        ...
        if (nevents) {
            if (_trace)
                fprintf (stderr, "ZMQ_POLL_TRACE: exit nevents=%d\n", nevents);
            break;
        }
        ...
        if (timeout_ < 0) {
            if (first_pass)
                first_pass = false;
            if (_trace)
                fprintf (stderr, "ZMQ_POLL_TRACE: no events, looping (infinite)\n");
            continue;
        }
        ...
        if (first_pass) {
            ...
            first_pass = false;
            if (_trace)
                fprintf (stderr, "ZMQ_POLL_TRACE: first pass done, end=%llu\n",
        ...
        now = clock.now_ms ();
        if (_trace)
            fprintf (stderr, "ZMQ_POLL_TRACE: check timeout now=%llu end=%llu\n",
                     (unsigned long long) now, (unsigned long long) end);
        if (now >= end)
            break;
    }
